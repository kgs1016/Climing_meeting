-- ═══════════════════════════════════════════════════════════════
--  기반 스키마 — 빈 DB 를 여기서부터 세운다
-- ═══════════════════════════════════════════════════════════════
-- 원래 supabase/schema.sql 에만 있던 내용이다. 마이그레이션에는 없어서
-- 빈 DB 에 db push 하면 첫 줄(alter table profiles …)에서 바로 깨졌다.
--
-- 테스트용 프로젝트를 새로 만들어 db push 한 번으로 세울 수 있게, 그리고
-- "빈 DB 에서도 전부 올라가는지" 가 매번 검증되게 여기로 옮긴다.
--
-- 이미 돌아가고 있는 DB 에 다시 실행돼도 안전하다 (schema.sql 자체가
-- 몇 번을 돌려도 되게 쓰여 있다). 그래도 본 DB 에서는 굳이 다시 돌리지 말고
-- 아래 한 줄로 "적용됨" 표시만 해두면 된다.
--
--   npx supabase migration repair --status applied 20260801000000
--
-- 여기 정의된 함수들은 뒤 마이그레이션에서 여러 번 갈아엎힌다. 최신 정의는
-- 항상 파일명이 가장 늦은 쪽이다.
